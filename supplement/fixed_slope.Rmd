...
1) glmGamPoi 
2) offset with $\theta=10$
3) glmGamPoi with slope fixed (log(10))
4) glmGamPoi with intercept fixed as in the offset model
5) With both fixed slope and fixed intercept 
...
plot_model_pars(vst_out[["glmGamPoi-offset"]], show_theta=TRUE)
residualVarPlot(vst_out[["glmGamPoi-offset"]])
```
...
```{r, fig.width=12, fig.height=6, out.width='95%'}
plot_lst <- lapply(names(vst_out), function(method_name) {
  plot_model_pars(vst_out[[method_name]], show_theta = TRUE) + ggtitle(method_name)
})
